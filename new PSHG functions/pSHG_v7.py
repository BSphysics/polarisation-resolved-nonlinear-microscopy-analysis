...
same as v6, so any 123D profile reproduces v6 output exactly.

Reproduction check before retiring v6: run '123D - forward SHG' on a dataset
already analysed under v6 and diff the saved I2_masked / Phi2_masked arrays.
They should be identical.

111 profiles ship with delta/flip = None and will refuse to run until the
half-wave-plate zero and rotation sense are calibrated in instruments.py.

@author: Ben (v7 refactor)
"""
import os
scriptDir = os.getcwd()
import sys
...
# Create results folder inside data_path
...
# ============================================================ analyse this FOV
# Pure compute (channel pick, projection, angle convention) lives in analyse_fov.
...
angles    = res['angles_gof']        # v6 passed the un-flipped vector downstream
